...
export async function reorderBlogs(req: Request, res: Response): Promise<void> {
	try {
		...
		for (const { id, blogId_New } of data) {
			if (
				typeof id !== "number" ||
				typeof blogId_New !== "number" ||
				Number.isNaN(id) ||
				Number.isNaN(blogId_New)
			) {
		...
		await Promise.all(
			data.map(({ id, blogId_New }) =>
				db.execute(
					sql`UPDATE ${blogTable} SET order_id = ${blogId_New} WHERE id = ${id}`,
				),
			),
		);
		...
		res
			.status(200)
			.json({ success: true, message: "Blogs reordered successfully" });
		return;
	} catch (error) {
...
export async function getBlogsInOrder(
	_req: Request,
	res: Response,
): Promise<void> {
	try {
		const blogs = await db
			...
			.from(blogTable)
			.orderBy(sql`${blogTable.orderId} ASC`);
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to fetch blogs in order" });
	}
}
