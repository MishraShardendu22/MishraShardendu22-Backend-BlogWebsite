...
if (SENDGRID_KEY) {
	...
	console.warn(
		"[WORKER] SENDGRID_API_KEY not set - worker will use fallback transporter",
	);
}
...
const transporter = {
	sendMail: async (mailOptions: any) => {
		...
		return Promise.resolve({
			messageId: "simulated-12345",
			response: "Simulated send success",
		});
	},
};
	...
	async (job) => {
		...
		try {
			if (SENDGRID_KEY) {
				...
				console.log(
					`[WORKER] SendGrid response status for ${to}:`,
					response.statusCode,
				);
				return {
					success: true,
